...
const handMovementThresholdMin = 0.2;  // m/s for moderate hand gesture speed
const handMovementThresholdMax = 0.5;  // m/s for excessive hand gesture speed
...
// Analyze hand movement and adjust confidence
function analyzeHandMovementConfidence(landmarks) {
    if (lastHandPosition) {
        const currentPosition = getCenterOfLandmarks(landmarks, Array.from({length: landmarks.length}, (_, i) => i)); // All hand landmarks
        const dx = currentPosition.x - lastHandPosition.x;
        const dy = currentPosition.y - lastHandPosition.y;
        const movementSpeed = Math.sqrt(dx * dx + dy * dy);  // Calculate hand movement speed
        
        lastHandPosition = currentPosition;

        // Adjust confidence based on movement speed
        if (movementSpeed >= handMovementThresholdMin && movementSpeed <= handMovementThresholdMax) {
            return 1.2;  // Moderate speed increases confidence
        } else if (movementSpeed > handMovementThresholdMax) {
            return 0.4;  // Excessive speed decreases confidence
        }
    }

    // Store initial hand position if not set yet
...
for (const landmarks of results.multiFaceLandmarks) {
    ...
    // Calculate total face-related confidence
    let faceFrameConfidence = (smileConfidence + movementConfidence + lipConfidence + blinkConfidence + headPoseConfidence) / 5;

    // Hand confidence from the previous hand results
    let handConfidence = (totalConfidence / frameCount) || 1;  // Avoid NaN if no hand data
    let finalConfidence = (faceFrameConfidence + handConfidence) / 2;  // Combine face and hand confidence

    // Display final confidence score
    confidenceScoreElement.innerText = (finalConfidence * 100).toFixed(2) + '%';

    // Log confidence factors in the console
...
function onHandsResults(results) {
    if (results.multiHandLandmarks) {
        for (const landmarks of results.multiHandLandmarks) {
            ...
            let handMovementConfidence = analyzeHandMovementConfidence(landmarks);  // Analyze hand confidence
